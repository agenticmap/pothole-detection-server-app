...
| 2.9 | ✅ Shipped (negative result) | **VLM verification — measured at last.** `hybrid_v1.py` has shipped since 2.3b with 21 tests, all against *fake* verifiers: no VLM has ever seen a frame from this corpus, so every number behind the design is an assumption. Shipped: `scripts/vlm_eval.py`, a read-only harness scoring the labelled frames through the real production path and reporting the VLM's binary verdict, matched-recall curves for Stage 1 / VLM / blend, band tables, and a free blend-weight sweep from a cached run; plus `openrouter` and `ollama` as named backends (same stdlib client as `local_http`, no new dependency). Also settles the thresholds on 340 labels rather than 140: **auto-accept >0.75 fires on 5 frames of 5,615 and the one labelled frame there is a false positive; auto-reject <0.40 would discard 55 of the 65 known potholes.** Scores prioritise, they never auto-label. **Measured 2026-09-03** against `qwen2.5vl:3b` over all 340 labelled frames, 0 failures: the verdict carries **no usable signal** — recall 0.015 (1 of 65 potholes), precision 0.200 against a 0.191 base rate. Its confidence is 0.8 or 0.9 on 96% of frames so it cannot be thresholded, and the blend's best F1 is identical for every weight from 0.00 to 0.70. Its rationales negate the prompt's own definition rather than describing the image, so this condemns a 3B model under a self-answering prompt, not the design. The threshold findings are confirmed and turn out not to be a VLM question at all. The run also surfaced a crash in the production crop path that no stub could reach. See [`phase-2.9-vlm-verification.md`](./phases/phase-2.9-vlm-verification.md). |
...
| 2.11 | ✅ Shipped | **Making the console say what it knows.** Four rounds that look unrelated and share one cause: the pipeline knew things the screen would not tell you. **The map drew three kinds of record and painted all of them as circles** — sensor observations were coloured from `--severity-4`/`--severity-2`, the same variables the cluster layer uses, so a pothole observation and a Severe cluster resolved to the **bit-identical** `#8c491a`, and frames differed from observations by half a pixel of radius. Shape now carries the kind (circle = cluster, triangle = sensor event, square = camera frame) because **shape survives greyscale, which is the test colour cannot pass**, with the recolour moved onto the categorical `--review-class-*` palette `tokens.css` had already specified and only the review surface obeyed. Aggregate bubbles started below the largest individual tier, so "40 defects here" and "one defect here" could render identically; the ramp now starts above it and labels the count. **Twenty of 7,615 frames were stored sideways** — one app build predating `setOutputImageRotationEnabled(true)`, a clean cutover at 2026-08-19. **The first diagnosis blamed the ingest EXIF strip and was wrong: the app writes no EXIF at all** (0 of 300 sampled), so the pixel buffer's shape is the only orientation record there has ever been. Fixed at rest rather than at render, because every stored box is normalized against stored pixels — mean p **0.180 → 0.224**, frames with boxes **9 → 13** — plus an ingest guard for a future tag-writing client and a `t` rotate key for the next one-off. VLM plumbing landed **deliberately without a button**: migration 018's `vlm_verified_at` kept separate from `detected_at`, and a per-user `user_quota` that **fails closed**, the opposite of the device-keyed `rate_limit` — but at recall 0.015 and ~40 s/frame, a button that makes a triaging operator wait forty seconds for a confident wrong answer is worse than none. Finally the map popup gained **the photograph** (its own `Popup`, an abort bound to `close`, and a pure `reducePreview` for the click-A-then-B race) and an **Open full size** path needing a new `GET /api/v1/frames/{client_id}` — with its own `FrameDetailResponse`, because `ClusterFrameItem` requires `paired_observation_id` and the frames layer deliberately carries frames that paired with nothing. And the dock now admits the gap: a **Corroborated** KPI rendered as a ratio, `0 of 204`, from the same predicate `/potholes` applies, because 0 alone says nothing that `0 of 204` does. Fixed on the way past: `distinct_passes` was optional in TypeScript while the server never sent it, so the panel printed **"0 passes" for every cluster**. The photograph then exposed two latent popup-layout bugs, both measured before being touched: a **544px popup in a 702px map** fits on neither side of a mid-map marker, and MapLibre never repositions after choosing an anchor, so it overhung the bottom by 68px and **cut the "Open full size" button in half**; and a popup is left at `z-index: auto` while the dock and legend sit at `--z-overlay`, so the legend covered the close button and the image's right edge. Fixed with a `--z-popup` tier, a pure `popupPanOffset` that pans the map instead of shrinking the popup, and a pinned title/action either side of a scrolling middle — because the height cap alone would have scrolled that button out of reach and re-created the defect by another route. Then an operator reported the map "looks confused", and investigating found the visual grammar describing a pipeline the system does not have. **Only pothole-classed readings ever reach a cluster** — 0 of 3,599 cracks and 0 of 1,781 `not`, against 229 of 229 passed potholes — so 254 of 5,686 readings (4.5%) are members of anything, and nothing on screen said so. Four defects, two of them false statements: a third of the triangles (`not`-class) had **no legend row at all**; grey meant both "no useful class" and "unscored frame" with only the frame meaning explained, though **zero** readings are unscored; hollow was driven by `sensor_is_outlier` while the legend claimed it meant "reached no cluster", which is wrong **both ways** — 25 hollow triangles were members and ~4,971 solid ones were not; and the popup claimed an outlier was "excluded from clustering", false for those same 25, which entered by the member gate's frame-pairing path. Fixed by carrying `in_cluster` on the tile (membership is not derivable from anything already there), so **solid fed something and hollow fed nothing** on all three shapes — extended up to clusters, where a ring now means **not yet corroborated**, from the read path's own predicate guarded by a spec that parses `config.py` so the two languages cannot drift. Every circle is a ring today. Plus a Class chip group (pothole only by default) and a legend split into Severity / What it is / Hollow means. **Two bugs surfaced by building it:** the class filter was applied before `addClusterLayers` created the layer, so `setFilter` silently did nothing; and a `clip-path` triangle sets `border: 0`, making the hollow-triangle legend swatch **invisible**. Then: *"how come one observation shapes the cluster?"* The behaviour is right and the presentation was not. Forming takes one admitted reading (`cluster_min_points = 1`), and nothing could have joined it — the radius is not the advertised 25 m ceiling but `min(2 × accuracy_m, 25)`, which at a median 4.37 m accuracy is **6.9 m**, where only **94 of 254** admitted readings have any neighbour against 188 at 25 m. So 163 of 204 clusters are singletons, and that is the correct unit: within one pass, two readings 14 m apart are two rough spots, not one defect twice. **No threshold moved**, because none can help — the 2σ buffer does omit the dominant error term (56% of readings carry whole-second timestamps, ±12.4 m of along-track error at 12.38 m/s against the 8.7 m allowed), but cross-day recurrence is 2/5/5/11/17 readings at 7/20/25/50/100 m, so the same defect is essentially never detected twice and repeatability is the binding constraint. What was wrong was the panel: `confidence` is `GREATEST(sensor_p_pothole, max_fused)`, so on a one-member cluster it is the saturating GMM posterior, and printing it as **Confidence 1.00** above **Observations 1** read as strength of evidence. Now a **Candidate / Corroborated** badge from a shared `isCorroborated` (lifted out of `map/layers.ts` so map, panel and dock cannot disagree), `Confidence` → **`Classifier score`** with the caveat on the row, and the corroboration note moved above the facts it qualifies — having gained the case it lacked, since it rendered *"All observations within 0 s"* for a set of **one**, on 163 of 204 clusters. Finally, the defect the guide had been carrying as a known limitation: **a `staff` account could not mark ANY real detection repaired**, because every cluster had `org_id IS NULL` and `repair_service` allows only an `admin` to repair an unowned one — a 403 on 100% of the corpus, reported as "Your account no longer has permission to do that". The cause was a deliberate refusal, not an oversight: migration 009 declined to backfill because there is no boundary table and (confirmed) no device→org mapping anywhere, so the server cannot infer an owner. Fixed by letting a deployment declare one — `CLUSTER_OWNER_ORG_ID`, unset by default, with the job stamping new clusters and filling NULLs via `COALESCE` so an existing owner is **never** reassigned, plus `scripts/assign_cluster_org.py` for the backlog. **`repair_service.py` was not touched**: the cross-org guard stands, the clusters simply have owners, and `repair_log.org_id` means something for the first time. A bad org id degrades to unowned rather than failing every insert and killing the job. 613 tests. See [`phase-2.11-console-legibility.md`](./phases/phase-2.11-console-legibility.md). |
| — | 📖 Record | **The console got a user guide, and the docs got a fifth genre.** Everything written until now addressed an engineer: the root README is server setup, `dashboard/README.md` is `npm install` and "things that will bite you", and the runbooks are pipeline procedures. Nothing told a person how to *use* the console. [`operator-console.md`](./guides/operator-console.md) covers both halves — map triage and the frame review loop — with both key maps reproduced from `keys.ts`, a table of what every on-screen number actually means, and a troubleshooting table for the behaviour that looks broken and is not. It is **rendered into the console itself** as the Help link by `dashboard/scripts/build-guide.mjs` on prebuild, following the `copy-maplibre-worker.mjs` precedent (generated, gitignored, copied to `dist/` by Vite), so there is exactly one copy and it cannot drift. **Writing it turned up a real defect:** every cluster the pipeline produces has `org_id IS NULL`, and `repair_service` requires `admin` for an unowned cluster — so a `staff` account **cannot mark any real detection repaired**, and the 403 surfaces as "Your account no longer has permission to do that", which misdescribes the cause. Documented, not patched. |
